...
function AffirmationSearch({ 
  ...
  const generateAffirmation = async (prompts: string[]) => {
    ...
    try {
      ...
      console.log('Queueing TTS job...');
      const queueResponse = await fetch('/api/queue-tts', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!queueResponse.ok) {
        ...
        console.error('Failed to queue TTS job:', errorData);
        throw new Error(`Failed to queue TTS job: ${errorData.message || queueResponse.statusText}`);
      }

      const { jobId } = await queueResponse.json();
      console.log('TTS job queued, jobId:', jobId);

      toast({
        description: "TTS conversion started. This may take a few minutes.",
      });

      console.log('Starting to poll TTS job...');
      const audioUrl = await pollTTSJob(jobId);
      
      if (!audioUrl) {
        console.warn('Server-side TTS failed, using Web Speech API fallback');
        const fallbackUrl = await fallbackTTS(newAffirmations.join('. '));
        setAffirmationAudioUrl(fallbackUrl);
        onAffirmationGenerated(newAffirmations, fallbackUrl);
        toast({
          description: "TTS conversion completed using fallback. You can now play the audio.",
        });
      } else {
    ...
    <div className="w-full">
      ...
      <div className="flex items-center space-x-2 mb-4">
        ...
        <Button 
          ...
          className="bg-purple-500 hover:bg-purple-600 text-white font-bold transition-all duration-300 ease-in-out"
        >
          Add
        </Button>
        <DropdownMenu>
    ...
    <div className="flex flex-col h-full">
      <div className="bg-white p-4 rounded-xl shadow-md mb-4 flex flex-col justify-between h-[200px]">
        <Select value={selectedTrack || undefined} onValueChange={handleTrackChange}>
          <SelectTrigger className="w-full bg-purple-100 border-purple-300 text-purple-800 hover:bg-purple-200 transition-colors duration-300">
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <Button 
        ...
        className="w-full mt-4 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white font-bold transition-all duration-300 ease-in-out"
      >
        Convert to Speech
      </Button>
    </div>
...
export const Studio: React.FC = () => {
  ...
  const [trackDuration, setTrackDuration] = useState(300); // Default to 5 minutes
  ...
  const handleDownloadAudio = async () => {
    if (!ttsAudioUrl || !selectedBackingTrack) {
      toast({
        description: 'TTS audio or backing track is missing. Please generate affirmations and select a backing track first.',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      const response = await fetch('/api/combine-audio', {
        ...
        body: JSON.stringify({
          ...
          ttsDuration, // Duration of one TTS loop
          ttsSpeed: playbackRate,
          loopDuration: selectedDuration * 60 // Should be the same as trackDuration
        }),
      });
      ...
      a.href = url;
      a.download = `combined_affirmation_audio_${selectedDuration}min.mp3`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
      toast({
        description: `Combined audio (${selectedDuration} minutes) downloaded successfully`,
      });
    } catch (error) {
  ...
  // Update the handleCreateFinalAudio function
  const handleCreateFinalAudio = async () => {
    ...
    try {
      console.log('Sending request with:', {
        ttsAudioUrl: ttsAudioUrl.substring(0, 50) + '...',
        backingTrackUrl: selectedBackingTrack,
        ttsVolume,
        backingTrackVolume,
        trackDuration,
      ...
      // Save the blob to IndexedDB or other suitable storage
      await saveBlobToStorage('finalAudioBlob', blob);

      // Save metadata about the audio
      setClientCache('finalAudioMetadata', {
        fileName: `final_audio_${Math.floor(trackDuration / 60)}min.mp3`,
        mimeType: blob.type,
      }, 3600); // Cache for 1 hour

      toast({
        description: `Final audio (${Math.floor(trackDuration / 60)} minutes) created and ready for download.`,
      });
    } catch (error) {
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <Slider
                        ...
                        value={[trackDuration]}
                        onValueChange={(newValue) => setTrackDuration(newValue[0])}
                        className="w-full"
                        trackClassName="h-1 bg-gray-200"
